...
class Hand {
	...
	determineRank(cards: Card[]): number {
		...
		// If any J's, add the J frequency to the highest card frequency.
		if (frequencies["J"] && Object.keys(frequencies).length > 1) {
			const js = frequencies["J"];

			delete frequencies["J"];

			const highestFrequency = Object.entries(frequencies).reduce(
				(previous, current) =>
					previous[1] > current[1] ? previous : current
			);

			frequencies[highestFrequency[0]] += js;
		}
